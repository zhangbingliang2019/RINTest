...
class RIN(nn.Module):
    def __init__(
            self,
            dim,
            image_size=(16, 256, 256),  # a tuple indicates image shape
            patch_size=(4, 8, 8),  # a tuple indicates patch shape
            channels=3,
        ...
        grid_size = [i // p for i, p in zip(image_size, patch_size)]
        self.grid_size = grid_size
        pixel_patch_dim = channels * patch_size[0] * patch_size[1] * patch_size[2]
        ...
        pos_emb_dim = dim // 2
        self.axial_pos_emb_frame_mlp = nn.Sequential(
            Rearrange('... -> ... 1'),
        ...
        self.to_pixels = nn.Sequential(
            ...
            Rearrange('b (f h w) (c p1 p2 p3) -> b c (f p1) (h p2) (w p3)',
                      p1=patch_size[0], p2=patch_size[1], p3=patch_size[2], f=grid_size[0], h=grid_size[1])
        )
    ...
    def forward(
        ...
        # if previous cond exists, directly use it otherwise initial with zeros
        ...
        # the warm starting of latents as in the paper (mlp + layer norm)
        ...
        frame_range = torch.linspace(0., 1., steps=self.grid_size[0], device=self.device)
        height_range = torch.linspace(0., 1., steps=self.grid_size[1], device=self.device)
        width_range = torch.linspace(0., 1., steps=self.grid_size[2], device=self.device)

        pos_emb_f, pos_emb_h, pos_emb_w = (self.axial_pos_emb_frame_mlp(frame_range),
                                           self.axial_pos_emb_height_mlp(height_range),
                                           self.axial_pos_emb_width_mlp(width_range))

        pos_emb = rearrange(pos_emb_f, 'f d -> f 1 1 d') + \
                  rearrange(pos_emb_h, 'i d -> 1 i 1 d') + \
                  rearrange(pos_emb_w, 'j d -> 1 1 j d')
        patches = patches + rearrange(pos_emb, 'f i j d -> (f i j) d')
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddpm_sample(self, cond, shape, time_difference=None):
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step', total=self.timesteps):
            ...
            model_output, last_latents = self.model(maybe_normalized_img, noise_cond, cond, x_start, last_latents,
                                                    return_latents=True)
    ...
    @torch.no_grad()
    def ddim_sample(self, cond, shape, time_difference=None):
        batch, device = shape[0], self.device
        ...
        for times, times_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            maybe_normalized_img = self.maybe_normalize_img_variance(img)
            model_output, last_latents = self.model(maybe_normalized_img, times, cond, x_start, last_latents,
                                                    return_latents=True)
    ...
    def sample(self, cond, batch_size=16):
        F, H, W, channels = *self.image_size, self.channels
        sample_fn = self.ddpm_sample if not self.use_ddim else self.ddim_sample
        return sample_fn(cond, (batch_size, channels, F, H, W))

    def forward(self, img, cond, *args, **kwargs):
        batch, c, f, h, w, device = *img.shape, img.device
        ...
        if random() < self.train_prob_self_cond:
            with torch.no_grad():
                model_output, self_latents = self.model(noised_img, times, cond, return_latents=True)
        ...
        pred = self.model(noised_img, times, cond, self_cond, self_latents)
...
class VideoTrainer(object):
    def __init__(
        ...
        dl = DataLoader(dataset, batch_size=train_batch_size, shuffle=False, pin_memory=True, num_workers=0)
